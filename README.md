...
| `detail` | string | No | `compact` (default) trims each chunk; `full` returns them whole. Split databases only: a unified answer is always whole |
...
verbatim and whole: it is never trimmed, capped or truncated, and `detail` does
not apply. It is markdown (`# Query results`, then `## Results`,
...
structured content, also whole (`chunks[]` with `context_id`, `score`, the
full `content`, `enrichment` (the enrichment text, a string, in full),
`enrichment_kind` (the declared `context_category`, present even when there is
no enrichment text) and `temporal[]` when the server sent it; `graph[]` with
`origin` (`query_path` or `chunk_relation`), `triplets[]` and `path_summary`;
