...
const hydraConfig: ConstructorParameters<typeof HydraDB>[0] = { token, database };

if (baseUrl != null) hydraConfig.baseUrl = baseUrl;
...
const resultOf = <Result>(result: Result) => toolResultSchema.parse(result);

const textOf = <Result>(result: Result) =>
	(resultOf(result).content ?? [])
		.map((c) => c.text ?? "")
		.join("\n");
...
console.log("   isError:", resultOf(f).isError === true);
...
console.log("   isError:", resultOf(bad).isError === true, "|", textOf(bad).slice(0, 120));
