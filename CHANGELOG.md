...
### Fixed — a recalled source now carries its date

`hydradb_query` rendered a source's title but dropped
`source_last_updated_time`, which the API returns on every chunk. A caller
asked to cite what it used could name a source but never say how current it
was, and could not tell a superseded document from the one that replaced it —
both match the same query, and only the date separates them. The Source line
now reads `Source: <title>  (updated YYYY-MM-DD)`. A source with a date but no
title is attributed rather than dropped; one with neither renders as before.
...
- `recency_bias` (0-1) was hardcoded to `0` with no override. That is still the
  ...
  read the dates rather than trusting the top hit.
